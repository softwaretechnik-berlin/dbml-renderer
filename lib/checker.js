...
const check = (input) => {
    const tables = extract("table", input).map((table) => ({
        actual: table,
        columns: table.items.filter((i) => i.type === "column"),
        indices: table.items.find((i) => i.type === "indices"),
        options: table.items.reduce((acc, i) => (i.type === "option" ? { ...acc, ...i.option } : acc), {}),
    }));
...
const extract = (type, output) => {
    return output.filter((entity) => entity.type === type);
};
