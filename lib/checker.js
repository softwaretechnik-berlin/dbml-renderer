...
const check = (input) => {
    ...
    const groups = extract("group", input).map((group) => ({
        ...
        tables: extract("table", group.items).map((i) => {
            ...
            const name = (0, common_1.tableName)(table.actual);
    ...
    const ungroupedTables = tables.filter((t) => !groupedTables.has((0, common_1.tableName)(t.actual)));
...
const resolveTable = (ref, tables) => {
    ...
    if (!table) {
        throw new Error(`Table ${(0, common_1.tableName)(ref)} does not exist`);
    }
    return table;
};
...
const extractColumns = (ref, tables) => {
    ...
    return {
        ...
        columns: ref.columns.map((c) => {
            ...
            if (!column) {
                throw new Error(`Column ${c} does not exist in table ${(0, common_1.tableName)(table.actual)}`);
            }
            return column;
        }),
    };
